package com.goodkredit.myapplication.responses.coopassistant;

import com.goodkredit.myapplication.model.coopassistant.member.CoopAssistantLoanTransactions;
import com.google.gson.annotations.Expose;
import com.google.gson.annotations.SerializedName;

import java.util.ArrayList;
import java.util.List;

public class CoopAssistantLoanTransactionsResponse {

    @SerializedName("data")
    @Expose
    private List<CoopAssistantLoanTransactions> coopAssistantLoanTransactions = new ArrayList<>();
    @SerializedName("status")
    @Expose
    private String status;
    @SerializedName("message")
    @Expose
    private String message;

    public List<CoopAssistantLoanTransactions> getCoopAssistantLoanTransactions() {
        return coopAssistantLoanTransactions;
    }

    public String getStatus() {
        return status;
    }

    public String getMessage() {
        return message;
    }
}
