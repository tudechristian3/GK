...
public class CoopAssistantSupportMessagesAdapter extends RecyclerView.Adapter<CoopAssistantSupportMessagesAdapter.MyViewHolder> {
    ...
    public CoopAssistantSupportMessagesAdapter(Context context) {
        mContext = context;
    ...
    public void onBindViewHolder(@NonNull final MyViewHolder holder, int position) {
        if (position > -1) {
            SupportConversation supportConversation = mGridData.get(position);

            holder.dateView.setText(CommonFunctions.convertDate(supportConversation.getDateTimeIN()));

            if (CommonFunctions.replaceEscapeData(supportConversation.getMessage()).contains("https://s3-us-west-1.amazonaws.com")) {
                holder.message.setVisibility(View.GONE);
                holder.image.setVisibility(View.VISIBLE);
                Glide.with(mContext)
                        .asBitmap()
                        .load(supportConversation.getMessage())
                ...
                holder.message.setText(CommonFunctions.replaceEscapeData(supportConversation.getMessage()));
    ...
    public int getItemViewType(int position) {
        SupportConversation supportConversation = mGridData.get(position);
        if (supportConversation.getThreadType().equals("0")) {
            return MY_MESSAGE;
        } else {
