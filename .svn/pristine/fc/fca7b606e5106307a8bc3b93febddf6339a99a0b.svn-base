...
//        try {
        ...
        voucherpin = findViewById(R.id.voucherpin);
        amount = findViewById(R.id.amount);
        remainingbalance = findViewById(R.id.remainingbalance);
        used = findViewById(R.id.used);
        barcode = findViewById(R.id.barcode);
        options = findViewById(R.id.options);
        linearView = findViewById(R.id.optionswrap);
        voucherimage = findViewById(R.id.voucherimage);
        transfervoucher = findViewById(R.id.transfervoucher);
        progress = findViewById(R.id.progress);
        generatepin = findViewById(R.id.generatepin);
        barcodeValue = findViewById(R.id.barcodeValueNi);
        whitebarcodeValue = findViewById(R.id.whitebarcodeValue);

        arrowdown = findViewById(R.id.arrowdown);
        voucherTag = findViewById(R.id.prepaid_tag);
        ...
        transfersms = dialog.findViewById(R.id.transferSMS);
        transferemail = dialog.findViewById(R.id.transferEMAIL);
        transferborrower = dialog.findViewById(R.id.transferBorrower);
...
//        try {
        ...
        if (cursor.getCount() > 0) {
            ...
            //setting the logo
            aq.id(voucherimage).image(CommonVariables.s3link + productidval + "-voucher-design.jpg", false, true);
        ...
        barcodesLayout = findViewById(R.id.barcodeLayout);
...
//        }
    ...
    public void verifySession() {
        //new HttpAsyncTask().execute(CommonVariables.GENERATENEWPIN);
        generateNewPinV2();
    ...
    Bitmap encodeAsBitmap(String contents, BarcodeFormat format, int img_width, int img_height) throws WriterException {
        ...
        BitMatrix result = writer.encode(contentsToEncode, format, img_width, img_height, hints);
    ...
    private void generateNewPinV2() {
        if (CommonFunctions.getConnectivityStatus(getViewContext()) > 0) {
            ...
            regenKeyEncryption = CommonFunctions.getSha1Hex(regenAuthenticationid + sessionid + "generateNewPinV2");
            regenParam = CommonFunctions.encryptAES256CBC(regenKeyEncryption, String.valueOf(paramJson));

            generateNewPinV2Object(generateNewPinV2Callback);
    ...
    private void generateNewPinV2Object(Callback<GenericResponse> renameVoucher) {
    ...
    private Callback<GenericResponse> generateNewPinV2Callback = new Callback<GenericResponse>() {
            @Override
            public void onResponse(Call<GenericResponse> call, Response<GenericResponse> response) {
                ...
                if (errorBody == null) {
                    ...
                    if (response.body().getStatus().equals("000")) {
                        ...
                        String vcode =CommonFunctions.parseJSON(data,"VoucherCode");
                        String vpin = CommonFunctions.parseJSON(data,"VoucherPIN");
                        String rbalance = CommonFunctions.parseJSON(data,"RemainingBalance");
                        String bcode = CommonFunctions.parseJSON(data,"BarCode");
