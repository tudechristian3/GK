...
public class MDPSupportNewCaseAdapter extends RecyclerView.Adapter<MDPSupportNewCaseAdapter.MyViewHolder> {
    private Context mContext;
    private List<MDPSupportHelpTopics> mGridData;

    public MDPSupportNewCaseAdapter(Context context) {
        mContext = context;
        mGridData = new ArrayList<>();
    }

    public void update(List<MDPSupportHelpTopics> data) {
    ...
    public void onBindViewHolder(@NonNull MyViewHolder holder, int position) {
        if (position > -1) {
            MDPSupportHelpTopics skycableSupportHelpTopics = mGridData.get(position);
...
//                        ((SkyCableActivity) mContext).displayView(14, args);
                        ((MDPSupportActivity)mContext).displayView(2, args);
