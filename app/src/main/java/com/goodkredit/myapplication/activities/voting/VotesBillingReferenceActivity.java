...
public class VotesBillingReferenceActivity extends BaseActivity implements View.OnClickListener {
    ...
    public static void start(Context context, VotesPayment votesPayment, String amount, String from) {
        Intent intent = new Intent(context, VotesBillingReferenceActivity.class);
        intent.putExtra(KEY_BILLING_ID, votesPayment.getMerchantReferenceNo());
        intent.putExtra(KEY_AMOUNT, amount);
        intent.putExtra(KEY_FROM, from);
        context.startActivity(intent);
    }

    public static void start2(Context context, VotesPending votesPending, String amount, String from) {
        Intent intent = new Intent(context, VotesBillingReferenceActivity.class);
        intent.putExtra(KEY_BILLING_ID, votesPending.getBillingID());
        intent.putExtra(KEY_AMOUNT, amount);
        intent.putExtra(KEY_FROM, from);
        context.startActivity(intent);
    }
    ...
    private void init() {
//        merchantname = PreferenceUtils.getStringPreference(getViewContext(),PreferenceUtils.KEY_MERCHANTNAME);
