...
public class SkycableNewApplicationRegistrationsFragment extends BaseFragment implements SwipeRefreshLayout.OnRefreshListener, View.OnClickListener {
...
//    private MaterialEditText edtMonth;
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        View view = inflater.inflate(R.layout.fragment_skycable_new_application_registrations, container, false);
    ...
    private void init(View view) {
        ...
        mAdapter = new SkycableNewApplicationRegistrationsAdapter(getViewContext(), false);
...
//                    Logger.debug("antonhttp", "======BOTTOM SCROLL=======");
    ...
    private void getSession() {
        if (CommonFunctions.getConnectivityStatus(getViewContext()) > 0) {
            ...
            mTvFetching.setText("Fetching SKYCABLE Registrations.");
...
//            call.enqueue(callsession);
            ...
            getSkycableNewApplicationRegistrations(getSkycableNewApplicationRegistrationsSession);
        } else {
...
//    };
    ...
    private void getSkycableNewApplicationRegistrations(Callback<GetSkycableNewApplicationRegistrationsResponse> getSkycableNewApplicationRegistrationsCallback) {
        Call<GetSkycableNewApplicationRegistrationsResponse> getskycablenewapplication = RetrofitBuild.getSkycableNewApplicationRegistrationsService(getViewContext())
                .getSkycableNewApplicationRegistrationsCall(sessionid,
                        imei,
                        userid,
                        borrowerid,
                        authcode,
                        ...
                        String.valueOf(year));
        getskycablenewapplication.enqueue(getSkycableNewApplicationRegistrationsCallback);
    }

    private final Callback<GetSkycableNewApplicationRegistrationsResponse> getSkycableNewApplicationRegistrationsSession = new Callback<GetSkycableNewApplicationRegistrationsResponse>() {

        @Override
        public void onResponse(Call<GetSkycableNewApplicationRegistrationsResponse> call, Response<GetSkycableNewApplicationRegistrationsResponse> response) {
            mLlLoader.setVisibility(View.GONE);
            mSwipeRefreshLayout.setRefreshing(false);
            ...
            if (errorBody == null) {
                if (response.body().getStatus().equals("000")) {
                    ...
                    isloadmore = response.body().getSkycableRegistrations().size() > 0;
                    isLoading = false;
                    isfirstload = false;

                    if (!isbottomscroll) {
                        mdb.truncateTable(mdb, DatabaseHandler.SKYCABLE_REGISTRATIONS);
                    }

                    List<SkycableRegistration> skycableRegistrationList = response.body().getSkycableRegistrations();

                    if (skycableRegistrationList.size() > 0) {
                        if (mdb != null) {
                            for (SkycableRegistration skycableRegistration : skycableRegistrationList) {

                                mdb.insertSkycableRegistrations(mdb, skycableRegistration);
                            }
                        }
                    }

                    updateList(mdb.getSkycableRegistrations(mdb));
        ...
        @Override
        public void onFailure(Call<GetSkycableNewApplicationRegistrationsResponse> call, Throwable t) {
            isLoading = false;
            mLlLoader.setVisibility(View.GONE);
            mSwipeRefreshLayout.setRefreshing(false);
            showToast("Something went wrong. Please try again.", GlobalToastEnum.NOTICE);
    ...
    private void updateList(List<SkycableRegistration> data) {
        if (data.size() > 0) {
