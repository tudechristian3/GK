...
import com.goodkredit.myapplication.adapter.projectcoop.ProjectCoopSupportFAQAdapter;
...
public class ProjectCoopSupportFAQFragment extends BaseFragment implements View.OnClickListener, ExpandableListView.OnGroupExpandListener {
...
//    private SwipeRefreshLayout mSwipeRefreshLayout;
    ...
    private ProjectCoopSupportFAQAdapter mAdapter;
...
//                    getProjectCoopSupportFAQ(getProjectCoopSupportFAQSession);
//                }
//            } else {
//                hideProgressDialog();
//                mLlLoader.setVisibility(View.GONE);
...
//            mSwipeRefreshLayout.setRefreshing(false);
    ...
    private void getProjectCoopSupportFAQ(Callback<GetSkycableSupportFAQResponse> getSkycableSupportFAQCallback) {
        Call<GetSkycableSupportFAQResponse> getskycablesupportfaq = RetrofitBuild.getGameSupportFAQService(getViewContext())
                .getProjectCoopSupportFAQCall(sessionid,
                        imei,
                        userid,
                        borrowerid,
                        authcode,
                        helptopic);
        getskycablesupportfaq.enqueue(getSkycableSupportFAQCallback);
    }

    private final Callback<GetSkycableSupportFAQResponse> getProjectCoopSupportFAQSession = new Callback<GetSkycableSupportFAQResponse>() {

        @Override
        public void onResponse(Call<GetSkycableSupportFAQResponse> call, Response<GetSkycableSupportFAQResponse> response) {
            mLlLoader.setVisibility(View.GONE);
//            mSwipeRefreshLayout.setRefreshing(false);
            ...
            if (errorBody == null) {
                if (response.body().getStatus().equals("000")) {
                    ...
                    List<SkycableSupportFAQ> skycableSupportFAQList = new ArrayList<>();
                    skycableSupportFAQList = response.body().getSkycableSupportFAQList();

                    expandableListTitle.clear();
                    expandableListDetail.clear();

                    for (SkycableSupportFAQ skycableSupportFAQ : skycableSupportFAQList) {
                        expandableListTitle.add(skycableSupportFAQ.getQuestion());
                        expandableListDetail.put(skycableSupportFAQ.getQuestion(), skycableSupportFAQ.getAnswer());
                    }

                    mAdapter = new ProjectCoopSupportFAQAdapter(getViewContext(), expandableListTitle, expandableListDetail);
        ...
        @Override
        public void onFailure(Call<GetSkycableSupportFAQResponse> call, Throwable t) {
            mLlLoader.setVisibility(View.GONE);
//            mSwipeRefreshLayout.setRefreshing(false);
            showToast("Something went wrong. Please try again.", GlobalToastEnum.NOTICE);
        }
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            case R.id.btnMessageUs: {
                ...
                ((ProjectCoopActivity) getViewContext()).displayView(6, args);
                break;
            }
        }
    }
