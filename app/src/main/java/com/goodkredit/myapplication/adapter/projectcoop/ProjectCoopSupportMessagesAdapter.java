...
public class ProjectCoopSupportMessagesAdapter extends RecyclerView.Adapter<ProjectCoopSupportMessagesAdapter.MyViewHolder> {
    private Context mContext;
    private List<SkycableConversation> mGridData;
    private static final int MY_MESSAGE = 0;
    private static final int OTHER_MESSAGE = 1;

    public ProjectCoopSupportMessagesAdapter(Context context) {
        mContext = context;
        mGridData = new ArrayList<>();
    }

    public void update(List<SkycableConversation> data) {
    ...
    public void onBindViewHolder(@NonNull final MyViewHolder holder, int position) {
        if (position > -1) {
            SkycableConversation skycableConversation = mGridData.get(position);

            holder.dateView.setText(CommonFunctions.convertDate(skycableConversation.getDateTimeIN()));

            if (CommonFunctions.replaceEscapeData(skycableConversation.getMessage()).contains("https://s3-us-west-1.amazonaws.com")) {
                holder.message.setVisibility(View.GONE);
                holder.image.setVisibility(View.VISIBLE);
                Glide.with(mContext)
                        .asBitmap()
                        .load(skycableConversation.getMessage())
                ...
                holder.message.setText(CommonFunctions.replaceEscapeData(skycableConversation.getMessage()));
    ...
    public int getItemViewType(int position) {
        SkycableConversation skycableConversation = mGridData.get(position);
        if (skycableConversation.getThreadType().equals("0")) {
            return MY_MESSAGE;
        } else {
